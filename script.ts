...
document.addEventListener('DOMContentLoaded', function() {
    // Elementos DOM
    const changeColorBtn = document.getElementById('changeColorBtn') as HTMLButtonElement;
    const newsContainer = document.getElementById('newsContainer') as HTMLDivElement;
    const loadMoreBtn = document.getElementById('loadMoreBtn') as HTMLButtonElement;
    const counterDisplay = document.getElementById('counter') as HTMLSpanElement;
    const searchBtn = document.getElementById('searchBtn') as HTMLButtonElement;
    const searchInput = document.getElementById('searchInput') as HTMLInputElement;
    const breakingText = document.getElementById('breaking-text') as HTMLParagraphElement;
    const subscribeBtn = document.getElementById('subscribeBtn') as HTMLButtonElement;
    const newsletterEmail = document.getElementById('newsletterEmail') as HTMLInputElement;
    const categoryItems = document.querySelectorAll('.nav-menu li');
    
    // Estado da aplicação
    let darkMode: boolean = false;
    let currentPage: number = 1;
    let newsPerPage: number = 6;
    let currentCategory: string = 'all';
    let newsDisplayed: number = 0;
    let breakingNewsIndex: number = 0;
    ...
    const breakingNewsArray: BreakingNews[] = [
    ...
    function setDarkModeFromPreference(): void {
    ...
    function updateDarkModeUI(): void {
        ...
        const icon = changeColorBtn.querySelector('i') as HTMLElement;
        if (icon) {
    ...
    function createNewsCard(news: NewsItem): HTMLDivElement {
        ...
        const favoriteBtn = newsCard.querySelector('.favorite') as HTMLButtonElement;
        favoriteBtn.addEventListener('click', function() {
            toggleFavorite(parseInt(this.dataset.id as string));
        });
        
        const readMoreBtn = newsCard.querySelector('.read-more-sm') as HTMLButtonElement;
    ...
    // Favoritos
    function toggleFavorite(newsId: number): void {
        const index = mockNews.findIndex(news => news.id === newsId);
        if (index !== -1) {
            mockNews[index].favorite = !mockNews[index].favorite;
            
            // Atualiza ícone no DOM
            const favoriteBtn = document.querySelector(`.favorite[data-id="${newsId}"]`) as HTMLButtonElement;
            if (favoriteBtn) {
                const icon = favoriteBtn.querySelector('i') as HTMLElement;
    ...
    function startBreakingNewsRotation(): void {
    ...
    function updateBreakingNews(): void {
    ...
    function flashBreakingNews(): void {
        const container = breakingText.parentElement as HTMLElement;
        let count = 0;
    ...
    function updateNewsCounter(): void {
    ...
    function setupEventListeners(): void {
        ...
        const mainReadMore = document.querySelector('.read-more') as HTMLButtonElement;
        if (mainReadMore) {
        ...
        const mainFavorite = document.querySelector('.favorite[data-id="1"]') as HTMLButtonElement;
        if (mainFavorite) {
        ...
        if (heading) {
            heading.addEventListener('dblclick', function() {
                const colors: string[] = ['#3498db', '#e74c3c', '#2ecc71', '#f39c12', '#9b59b6'];
